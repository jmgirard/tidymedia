...
* Added `standardize_video()`, a task verb that re-encodes a video to a
  ...
  H.264 video with `yuv420p` and `+faststart`, leaving resolution and frame
  rate untouched, so the same input always yields a byte-identical command.
  Supply `width`/`height` to set the output size (giving only one preserves the
  aspect ratio with an even output dimension), `fps` to resample the frame
  rate, and `vcodec`/`pixel_format` to override the codec or pixel format.
