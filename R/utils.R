...
check_paths_readable <- function(x, arg = rlang::caller_arg(x),
  ...
  # review F1). unreadable_paths() coerces too, defensively -- no live caller
  # needs it to, check_batch_inputs() having coerced each carrier already
  # (R/ffmpeg.R), but the predicate is reachable without passing through here
  # -- and this line stays because the coerced vector is what the message and
  # the return value carry.
