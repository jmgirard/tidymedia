...
2.  Run `tidymedia::install_on_win()` in R and confirm the prompt, which
    ...
    will unpack into, and the program locations it will overwrite. The
...
ffm(video, "output.mp4") |>
  ...
  ffm_compile()
#> [1] "-y -i \"/private/var/folders/px/frfvbz4n0sx90__c62fwwzz40000gn/T/RtmpZIiL48/temp_libpath1ed462f51f7/tidymedia/extdata/sample.mp4\" -vf \"trim=start=1:end=5,setpts=PTS-STARTPTS,crop=w=160:h=120:x=(in_w-out_w)/2:y=(in_h-out_h)/2\" -codec:v libx264 -an \"output.mp4\""
```
...
#> [1] "-y -i \"/private/var/folders/px/frfvbz4n0sx90__c62fwwzz40000gn/T/RtmpZIiL48/temp_libpath1ed462f51f7/tidymedia/extdata/sample.mp4\" -codec:a copy -vn -map \"0:a:0\" \"audio.aac\""
```
