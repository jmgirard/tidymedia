...
These functions also take a `quality` argument. It is the encoder’s own
rate-control value, passed through unchanged. `libx264` and `libx265`
read it as `-crf` (0 to 51). The nvenc encoders read it as `-cq` (0 to
51), and the videotoolbox encoders as `-q:v` (1 to 100). Each scale is
its own, so pick the number for the encoder you named:

``` r

standardize_video("raw.mp4", "std.mp4", hardware = "videotoolbox", quality = 60)
```

When `fallback = TRUE` falls back to software, the value is dropped and
the message says so. Leave `quality` as `NULL` to keep the encoder’s own
default.
