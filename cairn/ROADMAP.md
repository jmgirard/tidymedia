...
_Last hygiene check: 2026-07-26 (full audit; board clean, 2 pre-existing scaffold FAILs tracked for /cairn-init repair)_
...
- Tracking-scaffold hygiene (`/cairn-init` repair): add `.gitignore` entry `cairn/.merge-approved.pending`, rename the superseded `cairn/references/pdf/` ignore + dir to `sources/`, and backfill `**Provenance.**` blocks on `references/naming-docs-audit-M22.md` + `verb-coverage-survey-M25.md` — clears 2 pre-existing `cairn_validate` FAILs. — added 2026-07-26 — surfaced during M31 review; chip task_71add82f
