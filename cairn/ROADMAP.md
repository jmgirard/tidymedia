...
_Last hygiene check: 2026-07-29 (M41 + M42 planned from the M40 candidate row, which was corrected on promotion — one silent-`NA` site, not three; the row's other two verbs already abort. M41 takes the missing/misplaced front-door guards, M42 takes the `NULL`/`NA` semantics D021 left open. Terminal rows at 5, no pruning needed.)_
