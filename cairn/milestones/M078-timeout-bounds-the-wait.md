...
- 2026-08-28: review — all five criteria verified with fresh evidence (the container grid and the three excluded fixture files both re-measured at review, not read off the implementation's record); consistency gate clean; five findings, two fixed at the gate, one deferred, two rejected; no defect return and no amendment return.
...
At the merge gate the maintainer chose "fix both, then merge".

- **F1 — fixed on the branch.** `R/tidymedia-package.R:53-55` now reads "warn
  once at the end of the run saying how many jobs timed out and did not run to
  completion", which is what `R/ffm_batch.R:192` signals; the paragraph was
  rewrapped and `man/tidymedia-package.Rd` regenerated by `document()`.
- **F2 — fixed on the branch.** D056's falsifier now reads "a spawn returning
  materially later than limit + 40 s on any platform -- a different escalation,
  not the sub-second package frame around the wait that puts C1 at 42.41 s".
  The entry was unmerged, so this is an amendment before landing rather than an
  edit to history.
- **F3 — deferred** to the floor-script hardening candidate row, which already
  holds the `data-raw` instrument shapes from M074, M076 and M077.
- **F4 — rejected**, then moot: the ragged `NEWS.md` rewrap is a formatter's
  business, and the `R/tidymedia-package.R` paragraph F1 touched was rewrapped
  as part of that fix anyway.
- **F5 — rejected.** Two comment lines at 83-84 columns, in files already
  carrying such lines on untouched rows; a linter's business, and no stated
  convention is broken.

Re-verified after the fixes: `NOT_CRAN=true devtools::test()` **FAIL 0 | WARN 12
| SKIP 5 | PASS 6692**, unchanged; `document()` leaves the tree clean apart from
the regenerated `man/tidymedia-package.Rd` committed with it.
