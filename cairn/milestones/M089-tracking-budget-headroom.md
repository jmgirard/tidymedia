...
- 2026-08-29: review gate — the maintainer directed F1 fixed now (D067's stale ROADMAP after-figure corrected in place, the entry not yet merged) and F2 filed as a successor candidate row; merge of PR #93 approved. Both landed before the approval marker was written.
...
- **AC4 — pass.** `wc -l -c cairn/ROADMAP.md` reported 43 lines / 21,766 bytes at first measurement, against the criterion's 44 and 22,000, measured with the M089 row and the replacement hygiene stamp both in the file. Re-measured after the gate-directed F2 row landed: 44 lines / 21,981 bytes, still inside both figures — the row was paid for by compressing the memoization and `normalize_audio()` rows, and the stamp was resolved to its own byte count by fixed point.
