...
CI, first two runs (`adf305b` and earlier): every leg green except `windows-latest`, 9 failures all in `test-program-management.R` — F4 confirmed, by a mechanism the reviewer did not name. Under one `R_USER_CONFIG_DIR` both libraries resolve to `<root>/R/tidymedia` on Windows (rappdirs' win layout inserts the `"R"` appauthor where tools' inserts `"R"` for every platform), so the "differ" guard failed and the third state's `unlink()` deleted the only file (measured from the job log; reproduced locally with `rappdirs::user_config_dir(..., os = "win")`). Fix on the branch: the legacy side is redirected by a `local_mocked_bindings()` mock of `rappdirs::user_config_dir()` whose return value encodes the arguments it was called with, so the F1 pin now asserts the helper asked for `("tidymedia", "R")`; the new side keeps the envvar. Local: the file passes; the mutant dropping the appauthor fails one expectation. The stub instrument itself held on Windows (AC1 and AC3 passed there). Re-verification on CI and re-approval below.
