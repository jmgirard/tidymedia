...
Maintainer triage at the gate (2026-09-02): fix now — O1, O2, P1, O8.
Rejected with reason — O3 (no defect with today's only caller; the seam's
signature is wider than its use), O4 (the chosen contract, documented in
`@return` and pinned by AC2), O5 (verified working at cli 3.6.6; no failure to
fix), O6 (explicitly Out of scope and already carried by a candidate row),
O7 (machine-specific `build_readme()` output, not introduced by this change).
Nothing deferred to a new candidate row.

### Fixes made at the gate

- O1: `tm_confirm()` now routes a `utils::menu()` failure through the same
  `refuse()` that the `rlang::is_interactive()` check uses, so a session where
  the two predicates disagree gets the classed refusal rather than menu()'s
  unclassed error. Re-verified at the console: `options(rlang_interactive =
  TRUE)` under `Rscript` now aborts with `tidymedia_confirmation_unavailable`.
  New test "tm_confirm() refuses the same way when menu() itself cannot ask".
- O2: `tm_install_details()` split out of `tm_install_prompt()`, and
  `install_on_win()` passes the same items to the refusal as cli bullets,
  brace-escaped through the new `tm_cli_escape()` so a path containing braces
  is shown rather than interpolated (M44). The refusal now names the URL, the
  directory and the three `<program>_location.txt` files; asserted in AC1's
  test over a directory named `an install {dir}`. Roxygen and the `NEWS.md`
  entry updated to match.
- P1: AC4's test now pins `basename(config_files)` to
  `paste0(registered, "_location.txt")`, so a wrong `tm_config_file()` no
  longer moves both sides of the assertion together.
- O8: the `cli.width` comment now says what the option is for — AC4 names it,
  and it fences a formatter change rather than today's, which does not wrap.

Re-verified after the fixes, at the post-fix branch head: `devtools::test()`
`[ FAIL 0 | WARN 10 | SKIP 18 | PASS 11435 ]`; `devtools::check()` `Status: OK`;
`devtools::document()` no diff; `pkgdown::check_pkgdown()` no problems;
`cairn_validate.py` all checks passed; AC6's grep over the regenerated
`man/install_on_win.Rd` still reports 0. Every criterion's evidence above
holds at this head.
