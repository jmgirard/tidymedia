...
- **Principles touched:** —
- **Resolves:** —
- **Branch/PR:** `m103-install-leaves-dir-as-found`
...
- 2026-09-02: amendment executed — AC1 narrowed (created directories only; comparison-bound promises; targeted-entry leftover reporting), AC2 sharpened (corrected fixture facts, listing-readable guard, a nested subdirectory on a path the fixture writes into, two removal-seam mock cells), AC4 gained the pre-existing-file and pre-existing-directory clauses, AC7 added binding the refusal message the Goal's "or named" half promised and no criterion bound; Coverage gained `AC7 → T5, T6`. T3 reworded: R drops attributes on `NULL`, so the two-slot list is the only option the plan's alternative left standing.
## Decisions
