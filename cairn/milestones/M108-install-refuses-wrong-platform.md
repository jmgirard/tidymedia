...
- 2026-09-04: CI wait on PR #112 hit the harness ceiling and was stopped rather than left armed. Fresh state at that point: `pkgdown` pass, the six `R CMD check` legs and `test-coverage` still pending on head 31c2ff8, which the gate-directed test change re-triggered; the previous head 1800936 was green on all ten. Approval stands; the merge is not made. Full local suite 0 failures, 12261 passing, 18 skipped.
...
- 2026-09-04: step-7 approval: PR #112 approved for merge.
- 2026-09-04: CI wait on PR #112 hit the harness ceiling a second time and was stopped rather than left armed. Fresh state at that point, on a run re-triggered by the resume commit 9335dbc: `pkgdown` pass (2m28s) and `windows-latest (release)` pass (9m20s), the other six legs still pending. Approval marker `cairn/.merge-approved` is written and names PR #112; the merge is not made.
