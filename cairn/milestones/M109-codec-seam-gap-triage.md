...
- 2026-09-05: CI wait hit the harness ceiling with six of eight checks still in progress (macOS and pkgdown green); the watcher was stopped and the session stops before the merge. The approval marker is written and PR #113 is out of draft; resume with /milestone-review M109.
...
### Correction: the wrong-form cells record a refusal, never compiled bytes (2026-09-05)

Supersedes the "compiled bytes" clause of the ledger row (a) above, and corrects
the same clause in this file's `## Scope` **In** paragraph. Every one of the five
forms `tm_nvenc_wrong_forms()` holds is refused, so all 360 rows of the
`video_codec/<form>` class are aborts and no cell of it can ever compile. What the
class adds beside the blamed frame is therefore the refusal MESSAGE, which is what
the T3 mutant actually moved: twelve of its eighteen rows keep the verb's name and
change only what it says. Read the row's class and its measured evidence as
standing; read "compiled bytes" in it and in the Scope paragraph as "the refusal
recorded at that cell". The generator's own header (`:49`, `:178`) already hedges
correctly with "or its refusal", as does AC1, which is why no criterion turns on
this. Found by the review's diff-bug lens (F1) and corrected at the gate.
...
|---|---|---|---|
| F1 | [O] | Gap (a)'s stated purpose is not what the new cells measure: all 360 rows are aborts, because every one of the five wrong forms is refused, so what the class adds beside the blamed frame is the refusal MESSAGE and never compiled bytes. The Scope line and ledger row (a) both say "compiled bytes"; the generator's own header hedges correctly with "or its refusal", and AC1 is worded with the same "or" and is met. |**fix now** — corrected by the superseding `## Decisions` entry above; the criteria are unaffected. |
| F2 | [O] | `oot_jobs()`'s rename guard (`:83`, `file.exists(v[[2]])`) and the control that fences it (`:190`, `file.exists(v[[1]])`) test the same value, since row 2 is a copy of row 1 before renaming — so the control can never see a column the rename skipped. |**reject, measured** — a fix was attempted at the gate and reverted, because it did not close the blind spot the finding names. The guard skips when `v[[2]]` exists and the control when `v[[1]]` does, but those are the same value only for a column that EXISTS on disk; a rename skipped for any other reason is already caught by the old control. Keying the control on the rename's own record of what it touched moves the blind spot without closing it, since the skipped column still reads as an input. Measured 2026-09-05 across the seven jobs-taking members: every character column partitions cleanly — `input`/`main`/`overlay` exist, `output`/`audiofile`/`videofile` do not — so no output column exists on disk at spec-build time and the blind spot is unreachable. The sweep's own refusal-identity assertion is the load-bearing fence: a cell refused for colliding outputs carries a different sentence and reddens. |
| F3 | [O] | `oot_jobs()` rewrites any character column that is not a path (`:81-85` keys only on `is.character()` and `file.exists()`), so a jobs table growing a non-path character column would have that value silently rewritten. Not live: the seven reachable tables hold only paths plus list/numeric columns. |**reject** — not live; the seven reachable tables hold only paths plus list/numeric columns (measured 2026-09-05, same run as F2). |
| F4 | [O] | `oot_jobs(NULL, ...)` degrades silently: a member with a `jobs` formal but no `jobs` in its `tm_timeout_call_specs()` cell yields `NULL[c(1,1), ]` without error, where `oot_args()` has an explicit `stop()` for the analogous case (`:102-104`). |**reject** — unreachable: every jobs-taking reachable member has a `jobs` column in its spec cell, which the non-emptiness control at `:73-80` already asserts. |
| F5 | [O] | `args$video_codec <- NULL` (`:113`) deletes the argument rather than setting the sentinel — the `$<-` spelling T1 forbids and `nvenc_order_set_codec()` carries a comment against. Harmless because `batch_video_codecs()` ignores the scalar when the column exists. |**reject** — deletion is what this site wants, and the comment above it says the column replaces the scalar. The sibling file's rule guards passing an explicit NULL sentinel through `do.call()`, which is not what happens here. |
| F6 | [O] | AC2's 18 rows come from `nvenc_order_diff()`, which the generator (`:386`) calls "reported for the reader rather than for AC2"; `nvenc_order_contract_diff()` returns 0 under the same mutant. AC2's own wording names no comparison and is met. |**reject** — AC2 names no comparison and both were run and recorded: contract 0 rows, wide 18. The division of labour the generator states is unchanged. |
| F7 | [O] | The form-partition control (`:159-165`) derives its expected value with the same expression `oot_forms()` uses, so only the non-emptiness check at `:166` fences anything. The pre-existing check at `:128` has the same shape. |**reject** — pre-existing shape, matching the check at `:128`; the diff did not introduce it. |
| F8 | [O] | The comment at `data-raw/nvenc-probe-order-baseline.R:213` asserts "measured 2026-09-05, `[[<-` and `[<-` both STORE a `list(1)`", but T1's work-log entry records only grid counts, so no execution behind the claim is recorded. The lens re-derived it and it holds. |**reject** — the claim is true and was re-derived independently by the review lens; the work-log line recording only counts is a record-completeness point, not a defect in what shipped. |
| F9 | [O] | `sub("([^/]+)$", "2-\\1", ...)` (`:84`) assumes `/` separators. Latent: `file.path()`/`tempdir()` give `/` on Windows too. |**reject** — latent only; `file.path()` and `tempdir()` give `/` on Windows too, so no path this sweep builds can reach it. |
| F10 | [O] | The distinctness control checks within a column, never across them (`:188-192`), so two output columns given the same path would be renamed identically and still collide. |**reject** — not live: `separate_audio_video_batch`'s two output columns start distinct and each is renamed on its own basename, so they stay distinct. |
| F11 | [O] | `ns2` (`:157`) duplicates `ns`, already bound at `:126` in the same test body. |**reject** — a linter-class redundancy, out of scope at triage. |
| F12 | [S] | The jobs-column form is added to the out-of-table test and the domain-sanity test, but not to the fallback-message test at `:306-344`, which still runs the scalar form only. |**reject** — out of scope: the fallback-message test is tied to neither gap (a) nor gap (d), and was never covered by a prior milestone either. |
| F13 | [O] | Pre-existing: the `# Usage (from the package root):` header (`data-raw/nvenc-probe-order-baseline.R:76`) is orphaned from the usage block 27 lines below, and this milestone inserted a fourth provenance paragraph between them. |**reject** — pre-existing; the diff did not introduce the orphaned header. |
