...
- **AC7 — met.** `devtools::test()`: 12600 pass, 0 fail, 18 skip, 10 warnings (all pre-existing binary-probe noise). `devtools::document()` re-run produces no diff — `git status --porcelain` afterward shows only the milestone file this review is writing. `devtools::check()` (background run, `document = FALSE`, `--no-manual`): `Status: OK`, 0 errors, 0 warnings, 0 notes, 6m47.7s.
