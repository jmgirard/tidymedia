...
- [ ] T7: Drop directories from the listing, so a subdirectory whose own name ends
      in a listed extension is not an `input` row and `recursive = TRUE` is a
      superset of `recursive = FALSE`. Regression tests for both.
- [ ] T8: Refuse a multi-value `type` rather than silently using its first element,
      and make the extension refusal's noun phrase agree in number.
- [ ] T9: Close the test gaps review named — a directory that looks like a file, a
...
- [ ] T10: Narrow `NEWS.md`'s claim about which batch verbs take the returned table
      unaltered, and correct the workflow vignette's faked paths and the silent
      broadening in the metadata vignette.
- [ ] T11: ROADMAP candidate row for the three deferred findings (symlink
      resolution, hidden files, the `.ts`/`.ogv` gaps in the closed lists).
- [ ] T12: Re-run `devtools::check()`, `pkgdown::check_pkgdown()`, and the claim
