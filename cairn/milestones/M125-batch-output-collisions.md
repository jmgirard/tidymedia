...
- [ ] T3: Add the refusal to `segment_video_batch()`, `extract_frame_batch()` and `segment_video()`'s `outfiles`, moving `segment_video()`'s `outfiles` derivation (`R/ffmpeg.R:3875`) above `check_hardware_available()` (`:3872`); make any cell T1 shows red on `sample_frames_batch()`, `separate_audio_video_batch()` or the other sibling verbs pass.
