...
- [ ] AC3: `ffm_batch()` called directly aborts, naming the path, before `ffm_run()` is reached, under `run = FALSE` and `run = TRUE`, when two pipelines `.f` returns share an `output` — except outputs that write no file: `-`, a `pipe:` URL, or a pipeline whose output options set `-f null`. A table whose pipelines all take exempt outputs compiles and runs as before, and a table mixing exempt rows with a real repeat still aborts. Tested for each case.
- [ ] AC4: The help page of each export in AC1's domain, and of `ffm_batch()`, states the refusal in terms of that page's own destination argument or columns.
- [ ] AC5: `NEWS.md`'s development section carries one entry stating that the batch verbs, `segment_video()` and `ffm_batch()` refuse jobs sharing an output path, a table that previously ran now aborting; every behaviour it asserts is one AC1's or AC3's tests exercise.
...
- [ ] T1: Write the collision test first: `tests/testthat/test-batch-output-collision.R` plus a helper declaring one cell per form per export (AC1) and a control per cell (AC2). The domain comes from `tm_reaches(tm_call_graph(), e, "ffm_batch")` over the exports, with a completeness check failing on a domain export that has no cell. Stub the spawn primitives `helper-timeout-sweep.R` stubs and assert none is called; assert the destination in the message and the blamed export via `blamed_verb()` (`helper-blame.R`). Record in the work log which cells are red on `master`.
- [ ] T2: Add `reject_duplicate_outputs()` on the resolved outputs of `standardize_video_batch()`, `normalize_audio_batch()` and `anonymize_video_batch()`, above the dropped-track probe (`R/ffmpeg.R:5274`) and the two-pass analysis (`:5318`), leaving `reject_duplicate_inputs()` where D057 puts it; route `strip_metadata_batch()`'s inline check (`:4964`) through the shared helper.
- [ ] T3: Add the refusal to `segment_video_batch()`, `extract_frame_batch()` and `segment_video()`'s `outfiles`; make any cell T1 shows red on `sample_frames_batch()`, `separate_audio_video_batch()` or the other sibling verbs pass.
- [ ] T4: In `R/ffm_batch.R`, refuse repeated pipeline outputs after `.f` builds them and before any run, naming the path only, with the exemption rule from AC3; tests for refusal under both `run` values, the all-exempt table (execution cell `skip_if` FFmpeg is absent), and the mixed table.
- [ ] T5: Re-run the ordering and blame suites (`test-input-path-front-door.R`, `test-nvenc-front-door.R`, `test-hardware-out-of-table-blame.R`, `test-builder-blame-front-door.R`); where a new refusal moves a pinned precedence, log it in Decisions.
- [ ] T6: Roxygen on the 16 exports and `ffm_batch()` (and a line beside `ffm_jobs()`'s output-derivation example), `devtools::document()`, the `NEWS.md` entry; at review, read each of the 17 pages against AC4.
