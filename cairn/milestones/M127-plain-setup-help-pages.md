...
- R4-2: on the 9 `_batch` pages with an `audio_stream` column, "reads `NULL` this way" follows the column sentences, so "this way" points to "It does not fall back to the argument". New in `82b7c72d`.
...
- Dispositions (gate, 2026-09-13): R4-1, R4-2, R4-4 and R4-11 are fix now, because the branch added them. R4-3 and R4-5 to R4-10 are follow-up, in one candidate row in `cairn/ROADMAP.md`. None is rejected.
- Fix-now evidence: R4-1 now says that a function that starts no program gives no error, with the two examples. Measured with a bad limit: `extract_audio(run = FALSE)` gives the error, and `probe_video()` given a probe object and `has_hardware_encoder()` under the option do not. R4-2: `_batch` pages keep the full family sentence (render of `?segment_video_batch` checked). R4-4 now names only functions that write only a video file, with `standardize_video()` as the example the reviewer measured. R4-11 rewrapped. The ledger row for `tidymedia-package` is corrected. On the fixed tree: the sweep over 28 pages prints no finding, `devtools::test()` gives 1738 tests with 0 failed and 5 skipped, `devtools::check()` gives 0 errors, 0 warnings and 0 notes, `devtools::document()` makes no change, and `pkgdown::check_pkgdown()` finds no problems.
