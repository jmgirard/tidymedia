...
- [ ] T1: Tests first, then `check_batch_quality_col()` beside `check_batch_codec_col()` (`R/ffmpeg.R:5436`): numeric or all-`NA` logical, each non-`NA` cell checked with M135's `check_quality()` against that row's resolved encoder, wrapped in `check_batch_cell()` so the error names the row and never purrr's index.
