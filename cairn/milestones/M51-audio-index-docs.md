...
- 2026-07-31: Codecov reported a patch-coverage drop on the review-fix commit — the generator functions in `R/audio-stream-doc.R` run at `document()` time, not at package-use time, so no runtime test reached them. The profile makes coverage diagnostic-only and never a merge gate, but the drop pointed at a real gap: the gate answer chose the R-function mechanism partly because the text becomes directly unit-testable, and nothing was testing it. Added direct tests of `audio_stream_param()` and `audio_input_param()` asserting both family lists, the topic link, the family-specific `NULL` wording, the batch-only column sentence, and that verb-specific text lands before the closing pointer. Suite 3275 pass / 0 fail; `check()` still 0/0/0.
...
- **AC8** — re-run after the review fixes: `devtools::document()` leaves the tree
  clean apart from the milestone file; `devtools::test()` 3275 pass / 0 fail / 5 skip;
...
- **F11 (60) + F12 (50)** — the guard skipped under `R CMD check`, where `man/`
  ...
  pass / 5 skip — identical to `devtools::test()` at that commit, so the doc
  tests ran under check rather than skipping.
