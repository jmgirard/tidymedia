...
- 2026-08-07: minor amendment — T10 added at the user's direction after review, to close the three logged evidence gaps (F10/F11, F4, F12) before re-review. No criterion changes: the grid and tests get stronger, the promises they verify stay as written.
- 2026-08-07: T10 — F10/F11 closed: the vocabulary uniqueness guard now reads the loaded NAMESPACE instead of globbing `../../R`, so it runs under `R CMD check` (where that path holds only the lazy-load DB and the old test skipped) and carries no skip path at all; deparsing normalizes quoting, spacing and wrapping, so it no longer pins formatting. Split into two guards because a fourth copy has two possible homes: one scans every namespace function's BODY, the other asserts all six signatures default to the accessor call. Both mutation-verified — a literal formal default reddens the second and only the second, a stray body literal reddens the first and only the first.
