# Tests for anonymize_video_batch(): a table-driven sibling of anonymize_video() that
...
test_that("anonymize_video_batch() returns one command per job with its own regions", {
  f1 <- make_input()
  f2 <- make_input()
  jobs <- tibble::tibble(
    input   = c(f1, f2),
...
test_that("anonymize_video_batch() draws every box for a multi-region row", {
  f <- make_input()
  jobs <- tibble::tibble(
    input   = f,
    output  = "a.mp4",
...
test_that("anonymize_video_batch() command is byte-identical to the scalar verb", {
  f <- make_input()
...
test_that("anonymize_video_batch() honors per-row knob columns", {
  f <- make_input()
  jobs <- tibble::tibble(
    input        = c(f, f),
    output       = c("a.mp4", "b.mp4"),
...
test_that("anonymize_video_batch() knob column overrides the scalar arg per row", {
  f <- make_input()
  ...
  res <- anonymize_video_batch(jobs, color = "green", run = FALSE)
  expect_match(res$command[[1]], "c=red:t=fill", fixed = TRUE)
  expect_match(res$command[[2]], "c=white:t=fill", fixed = TRUE)
  expect_no_match(res$command[[1]], "c=green", fixed = TRUE)
})

test_that("anonymize_video_batch() scalar arg applies to every row without a column", {
  f <- make_input()
  jobs <- tibble::tibble(
    input   = c(f, f), output = c("a.mp4", "b.mp4"),
    regions = list(
...
test_that("anonymize_video_batch() honors a per-box color column inside a regions cell", {
  f <- make_input()
  jobs <- tibble::tibble(
  ...
  res <- anonymize_video_batch(jobs, color = "red", run = FALSE)
...
test_that("anonymize_video_batch() auto-names outputs when the column is absent", {
  f1 <- make_input()
  f2 <- make_input()
...
test_that("anonymize_video_batch() keeps the input extension when auto-naming", {
...
test_that("anonymize_video_batch() aborts on a duplicated input with no output column", {
  f <- make_input()
  jobs <- tibble::tibble(
  ...
  expect_error(anonymize_video_batch(jobs, run = FALSE), "[Dd]uplicated")
})

test_that("anonymize_video_batch() allows a duplicated input with an explicit output", {
  f <- make_input()
  jobs <- tibble::tibble(
    input   = c(f, f), output = c("a.mp4", "b.mp4"),
    regions = list(
...
test_that("anonymize_video_batch() rejects a non-data-frame jobs", {
  expect_error(anonymize_video_batch(list(input = "a"), run = FALSE), "data frame")
})

test_that("anonymize_video_batch() rejects an empty jobs table", {
  jobs <- tibble::tibble(input = character(), regions = list())
  expect_error(anonymize_video_batch(jobs, run = FALSE), "at least one row")
})

test_that("anonymize_video_batch() names the missing input column", {
  jobs <- tibble::tibble(
    output = "a.mp4",
    regions = list(data.frame(x = 0, y = 0, width = 10, height = 10))
  )
  expect_error(anonymize_video_batch(jobs, run = FALSE), "input")
})

test_that("anonymize_video_batch() names the missing regions column", {
  f <- make_input()
  jobs <- tibble::tibble(input = f, output = "a.mp4")
  expect_error(anonymize_video_batch(jobs, run = FALSE), "regions")
})

test_that("anonymize_video_batch() rejects a flat (non-list) regions column", {
  f <- make_input()
  jobs <- tibble::tibble(input = f, output = "a.mp4", regions = 5)
  expect_error(anonymize_video_batch(jobs, run = FALSE), "list-column")
})

test_that("anonymize_video_batch() rejects a non-character knob column", {
  f <- make_input()
  jobs <- tibble::tibble(
    input = f, output = "a.mp4",
    regions = list(data.frame(x = 0, y = 0, width = 10, height = 10)),
...
test_that("anonymize_video_batch() inherits per-region size checks from ffm_drawbox", {
  ...
  expect_error(anonymize_video_batch(jobs, run = FALSE),
               "single FFmpeg expression or number")
})

# Return-schema parity ----------------------------------------------------

test_that("anonymize_video_batch() return schema matches a direct ffm_batch call", {
  f <- make_input()
  regions <- data.frame(x = 0, y = 0, width = 10, height = 10)
  jobs <- tibble::tibble(input = f, output = "a.mp4", regions = list(regions))
  res <- anonymize_video_batch(jobs, run = FALSE)
  # anonymize_video_batch() is a thin wrapper over ffm_batch(), so its dry-run schema
...
test_that("anonymize_video_batch() writes anonymized outputs (binary-gated)", {
  skip_if_no_ffmpeg()
  a <- make_test_video()
  b <- make_test_video()
  ...
  res <- anonymize_video_batch(jobs)
...
test_that("anonymize_video_batch() forwards verify (binary-gated)", {
  skip_if_no_ffmpeg()
  a <- make_test_video()
  b <- make_test_video()
  out_a <- withr::local_tempfile(fileext = ".mp4")
  ...
  res <- anonymize_video_batch(jobs, verify = list(width = 64, height = 64))
  expect_true(all(res$success))
  expect_true("verified" %in% names(res))
  expect_true(all(res$verified))
})

test_that("anonymize_video_batch() forwards manifest, read by ffm_manifest (binary-gated)", {
  skip_if_no_ffmpeg()
  a <- make_test_video()
  b <- make_test_video()
  out_a <- withr::local_tempfile(fileext = ".mp4")
  ...
  res <- anonymize_video_batch(jobs, manifest = TRUE)
