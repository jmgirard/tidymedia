...
ordering_cases <- function(input) {
  ...
  # Three inputs, so `resize = TRUE` contradicts the input count while `audio_input`
  # stays free to be in or out of range independently of it. This is the only
  # contradiction an IN-RANGE or out-of-range `audio_input` can cross: supplying such
  # a value is what removes the audio_codec one. An NA-ish `audio_input` is the
  ...
  list(
    ...
    # `audio_input` on compare_videos_batch() is probed at BOTH bounds, because the
    ...
    # `audio_input` even by which bound was crossed", and probing one bound would miss
    # the half that moves.
    list(id = "audio-low/argument", verb = "compare_videos_batch",
         wins = resize2, other = range,
         args = list(jobs = cmp1x(), audio_input = -1, resize = TRUE),
         control = list(jobs = cmp1x(), audio_input = 0, resize = TRUE)),
    list(id = "audio-low/column", verb = "compare_videos_batch",
         wins = resize2, other = range,
         args = list(jobs = cmp2x(audio_input = c(-1, -1)), resize = TRUE),
         control = list(jobs = cmp2x(audio_input = c(0, 0)), resize = TRUE)),
    list(id = "audio-high/argument", verb = "compare_videos_batch",
         wins = resize2, other = range,
         args = list(jobs = cmp1x(), audio_input = 7, resize = TRUE),
         control = list(jobs = cmp1x(), audio_input = 0, resize = TRUE)),
    list(id = "audio-high/column", verb = "compare_videos_batch",
         wins = resize2, other = range,
         args = list(jobs = cmp2x(audio_input = c(7, 7)), resize = TRUE),
         control = list(jobs = cmp2x(audio_input = c(0, 0)), resize = TRUE)),

    # picture_in_picture_batch()'s `audio_input` reaches its verb's only
    ...
    list(id = "audio/column", verb = "picture_in_picture_batch",
         wins = no_audio, other = range,
         args = list(jobs = pip2(audio_input = c(NA, 9)), audio_codec = "aac"),
         control = list(jobs = pip2(audio_input = c(NA, 0)), audio_codec = "aac")),
    ...
    #
    # The CONTROL is `audio_input = NULL`, not an in-range index, and it has to be:
    ...
    list(id = "audio/argument", verb = "picture_in_picture_batch",
         wins = no_audio, other = range,
         args = list(jobs = pip1(), audio_input = NA, audio_codec = "aac"),
         control = list(jobs = pip1(), audio_input = NULL, audio_codec = "aac")),
    list(id = "audio-na/argument", verb = "compare_videos_batch",
         wins = no_audio, other = range,
         args = list(jobs = cmp1(), audio_input = NA, audio_codec = "aac"),
         control = list(jobs = cmp1(), audio_input = NULL, audio_codec = "aac")),
    ...
    list(id = "audio-na/column", verb = "compare_videos_batch",
         wins = no_audio, other = range,
         args = list(jobs = cmp2(audio_input = c(NA, 7)), audio_codec = "aac"),
         control = list(jobs = cmp2(audio_input = c(NA, 0)), audio_codec = "aac")),
    ...
    list(id = "audio-nan/argument", verb = "picture_in_picture_batch",
         wins = no_audio, other = range,
         args = list(jobs = pip1(), audio_input = NaN, audio_codec = "aac"),
         control = list(jobs = pip1(), audio_input = NULL, audio_codec = "aac")),
...
test_that("an NA-ish `audio_input` argument is what reaches pip's contradiction", {
  # The reachability condition AC1 names, pinned as its own test rather than
  # left implicit in the case list. `audio_input` is the one guard here whose value
  ...
  for (na_ish in list(NA, NaN)) {
    na_cell <- catch_call("picture_in_picture_batch",
                          list(jobs = jobs, audio_input = na_ish,
                               audio_codec = "aac"))
  ...
  # This is the fact an earlier draft over-generalized into "supplying `audio_input`
  # at all removes the contradiction", which is false at any NA-ish value.
  out_of_range <- catch_call("picture_in_picture_batch",
                             list(jobs = jobs, audio_input = 9, audio_codec = "aac"))
  ...
  ok <- catch_call("picture_in_picture_batch",
                   list(jobs = jobs, audio_input = 1, audio_codec = "aac"))
...
test_that("a value check still reports before nvenc availability, in both forms", {
  ...
  cases <- list(
    ...
    list(id = "audio/argument", verb = "compare_videos_batch",
         ...
         args = c(list(jobs = cmp(), audio_input = 7), gpu),
         control = c(list(jobs = cmp(), audio_input = 0), gpu)),
    ...
    list(id = "margin/column", verb = "picture_in_picture_batch",
         ...
         args = c(list(jobs = pip(margin = -3)), gpu),
         control = c(list(jobs = pip(margin = 16)), gpu)),
    # The new site: before this milestone pip's `audio_input` COLUMN had no
    # front-door guard, so this cell reported the encoder.
    list(id = "pip audio/argument", verb = "picture_in_picture_batch",
         wins = "must be a whole number", other = "nvenc",
         args = c(list(jobs = pip(), audio_input = 9), gpu),
         control = c(list(jobs = pip(), audio_input = 1), gpu)),
...
# --- AC4: pip's per-row `audio_input` index is checked at the front door -------------

test_that("picture_in_picture_batch's `audio_input` index is refused at its front door", {
  ...
  cases <- list(
    list(id = "argument", jobs = two(main = input, overlay = input,
                                     output = "o.mp4"), extra = list(audio_input = 9)),
    list(id = "column", jobs = two(main = input, overlay = input,
                                   output = "o.mp4", audio_input = 9),
         extra = list()),
    ...
    for (case in cases) {
      ...
      cnd <- catch_call("picture_in_picture_batch",
                        c(list(jobs = case$jobs, parallel = parallel),
                          case$extra))
      expect_s3_class(cnd, "rlang_error")
      expect_match(conditionMessage(cnd), "`audio_input` must be a whole number",
                   info = id)
...
test_that("a clean `audio_input` column still compiles every row", {
...
test_that("each term the sentence quantifies over has a cell", {
  ...
  # Every `_batch` pair carries both forms. `audio_input` reaches its contradiction
