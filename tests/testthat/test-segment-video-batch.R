# Tests for segment_video_batch(): a table-driven sibling of segment_video() that
...
test_that("segment_video_batch() returns one command per job across multiple inputs", {
  f1 <- make_input()
  f2 <- make_input()
  jobs <- tibble::tibble(
    input  = c(f1, f1, f2),
...
test_that("segment_video_batch() default reencode = TRUE is accurate output-seek, no copy", {
  f <- make_input()
...
test_that("segment_video_batch(reencode = FALSE) uses the fast copy path per row", {
  f1 <- make_input()
  f2 <- make_input()
  jobs <- tibble::tibble(
  ...
  res <- segment_video_batch(jobs, reencode = FALSE, run = FALSE)
...
test_that("segment_video_batch() rejects a non-data-frame jobs", {
  expect_error(segment_video_batch(list(input = "a"), run = FALSE), "data frame")
})

test_that("segment_video_batch() rejects an empty jobs table", {
  jobs <- tibble::tibble(input = character(), output = character(),
                         start = numeric(), end = numeric())
  expect_error(segment_video_batch(jobs, run = FALSE), "at least one row")
})

test_that("segment_video_batch() names the missing required column", {
  f <- make_input()
  ...
  expect_error(segment_video_batch(jobs, run = FALSE), "Missing column")
})

# Execution + ffm_batch forwarding (binary-gated) -------------------------

test_that("segment_video_batch() writes segments and forwards verify (binary-gated)", {
  skip_if_no_ffmpeg()
  a <- make_test_video()
  b <- make_test_video()
  ...
  res <- segment_video_batch(jobs, verify = list(width = 64, height = 64))
...
test_that("segment_video_batch() forwards manifest, read by ffm_manifest (binary-gated)", {
  skip_if_no_ffmpeg()
  a <- make_test_video()
  b <- make_test_video()
  out_a <- withr::local_tempfile(fileext = ".mp4")
  ...
  res <- segment_video_batch(jobs, manifest = TRUE)
...
test_that("segment_video_batch() auto-names outputs when the column is absent", {
  f <- make_input()
...
test_that("segment_video_batch() auto-naming restarts numbering per input file", {
  f1 <- make_input()
  f2 <- make_input()
...
test_that("segment_video_batch() uses an explicit output column unchanged", {
  f <- make_input()
  jobs <- tibble::tibble(
...
test_that("segment_video_batch() honors a per-row reencode column", {
  f <- make_input()
  jobs <- tibble::tibble(
...
test_that("segment_video_batch() reencode column overrides the scalar arg", {
  f <- make_input()
  jobs <- tibble::tibble(
    input = c(f, f), output = c("a.mp4", "b.mp4"),
    start = c(0, 2), end = c(5, 7), reencode = c(TRUE, FALSE)
...
test_that("segment_video_batch() rejects a non-numeric/character start column", {
  f <- make_input()
  jobs <- tibble::tibble(input = f, output = "a.mp4", start = TRUE, end = 5)
  expect_error(segment_video_batch(jobs, run = FALSE), "start")
})

test_that("segment_video_batch() rejects a non-numeric/character end column", {
  f <- make_input()
  jobs <- tibble::tibble(input = f, output = "a.mp4", start = 0, end = TRUE)
  expect_error(segment_video_batch(jobs, run = FALSE), "end")
})

test_that("segment_video_batch() rejects a non-logical reencode column", {
  f <- make_input()
  jobs <- tibble::tibble(
    input = f, output = "a.mp4", start = 0, end = 5, reencode = "yes"
  )
  expect_error(segment_video_batch(jobs, run = FALSE), "reencode")
})

test_that("segment_video_batch() rejects an NA in the reencode column at the front door", {
