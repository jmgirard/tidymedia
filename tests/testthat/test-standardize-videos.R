# Tests for standardize_videos(): a table-driven sibling of standardize_video()
...
test_that("standardize_videos() returns one command per job across multiple inputs", {
  f1 <- make_input()
  f2 <- make_input()
  jobs <- tibble::tibble(
    input  = c(f1, f1, f2),
  ...
  res <- standardize_videos(jobs, run = FALSE)
...
test_that("standardize_videos() command is byte-identical to the scalar verb", {
  f <- make_input()
...
test_that("standardize_videos() default knobs match the scalar defaults", {
  f <- make_input()
  res <- standardize_videos(tibble::tibble(input = f, output = "out.mp4"),
                            run = FALSE)
...
test_that("standardize_videos() honors per-row knob columns", {
  f <- make_input()
  jobs <- tibble::tibble(
    input        = c(f, f, f),
    output       = c("a.mp4", "b.mp4", "c.mp4"),
...
test_that("standardize_videos() knob column overrides the scalar arg per row", {
  f <- make_input()
  ...
  res <- standardize_videos(jobs, fps = 60, run = FALSE)
  expect_match(res$command[[1]], "fps=24", fixed = TRUE)
  expect_match(res$command[[2]], "fps=30", fixed = TRUE)
  expect_no_match(res$command[[1]], "fps=60", fixed = TRUE)
})

test_that("standardize_videos() scalar arg applies to every row without a column", {
  f <- make_input()
  jobs <- tibble::tibble(input = c(f, f), output = c("a.mp4", "b.mp4"))
  res <- standardize_videos(jobs, width = 512, vcodec = "libx265", run = FALSE)
...
test_that("standardize_videos() auto-names outputs when the column is absent", {
  f1 <- make_input()
  f2 <- make_input()
...
test_that("standardize_videos() keeps the input extension when auto-naming", {
  f <- make_input(ext = "mkv")
  base <- tools::file_path_sans_ext(f)
  res <- standardize_videos(tibble::tibble(input = f), run = FALSE)
  expect_equal(res$output, paste0(base, "_standardized.mkv"))
})

test_that("standardize_videos() aborts on a duplicated input with no output column", {
  f <- make_input()
  jobs <- tibble::tibble(input = c(f, f))  # same input twice, would collide
  expect_error(standardize_videos(jobs, run = FALSE), "[Dd]uplicated")
})

test_that("standardize_videos() allows a duplicated input with an explicit output column", {
  f <- make_input()
  jobs <- tibble::tibble(input = c(f, f), output = c("a.mp4", "b.mp4"))
  res <- standardize_videos(jobs, run = FALSE)
...
test_that("standardize_videos() uses an explicit output column unchanged", {
  f <- make_input()
...
test_that("standardize_videos() rejects a non-data-frame jobs", {
  expect_error(standardize_videos(list(input = "a"), run = FALSE), "data frame")
})

test_that("standardize_videos() rejects an empty jobs table", {
  jobs <- tibble::tibble(input = character())
  expect_error(standardize_videos(jobs, run = FALSE), "at least one row")
})

test_that("standardize_videos() names the missing input column", {
  jobs <- tibble::tibble(output = "a.mp4")
  expect_error(standardize_videos(jobs, run = FALSE), "input")
})

test_that("standardize_videos() rejects a wrong-type knob column", {
  f <- make_input()
  jobs <- tibble::tibble(input = f, output = "a.mp4", width = TRUE)
  expect_error(standardize_videos(jobs, run = FALSE), "width")
})

test_that("standardize_videos() rejects an NA in a dimension column at the front door", {
  f <- make_input()
  jobs <- tibble::tibble(
    input = c(f, f), output = c("a.mp4", "b.mp4"), fps = c(24, NA)
  )
  expect_error(standardize_videos(jobs, run = FALSE), "fps")
})

test_that("standardize_videos() rejects a non-character vcodec column", {
  f <- make_input()
  jobs <- tibble::tibble(input = f, output = "a.mp4", vcodec = 5)
  expect_error(standardize_videos(jobs, run = FALSE), "vcodec")
})

# Execution + ffm_batch forwarding (binary-gated) -------------------------

test_that("standardize_videos() writes standardized outputs (binary-gated)", {
...
test_that("standardize_videos() forwards verify (binary-gated)", {
  skip_if_no_ffmpeg()
  a <- make_test_video()
  b <- make_test_video()
  out_a <- withr::local_tempfile(fileext = ".mp4")
  ...
  res <- standardize_videos(jobs, verify = list(width = 64, height = 64))
  expect_true(all(res$success))
  expect_true("verified" %in% names(res))
  expect_true(all(res$verified))
})

test_that("standardize_videos() forwards manifest, read by ffm_manifest (binary-gated)", {
  skip_if_no_ffmpeg()
  a <- make_test_video()
  b <- make_test_video()
  out_a <- withr::local_tempfile(fileext = ".mp4")
  ...
  res <- standardize_videos(jobs, manifest = TRUE)
